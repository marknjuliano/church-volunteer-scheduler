console.log('Tech/Creative Schedule v1.0.0-alpha6.2.3');
...
function renderSetup(){appEl.innerHTML=`<div class="wrap login"><div><div class="hero brandHero loginBrandHero"><div class="brandLeft"><img src="images/tech-creative-logo.png" class="powerDinkLogo" alt="Tech/Creative logo"><span class="brandDivider"></span><div class="brandTitle"><h1>TECH/CREATIVE<br><span class="brandSubline">SCHEDULE</span></h1><p>v1.0.0 Alpha 4.1</p></div></div></div><div class="card"><h2>Connect Firebase First</h2><div class="notice warn"><b>This build is ready, but it is intentionally not connected to the old Pickleball database.</b></div><p>Open <code>js/firebase.js</code> and paste the Web App configuration from your new church Firebase project.</p><p class="small">This protects your existing PowerDink data from being mixed with church schedules.</p></div></div></div>`}
...
function renderLogin(){
  appEl.innerHTML=`<div class="wrap login"><div>
    <div class="hero brandHero loginBrandHero">
      <div class="brandLeft">
        <img src="images/tech-creative-logo.png" class="powerDinkLogo" alt="Tech/Creative logo">
        <span class="brandDivider"></span>
        <div class="brandTitle">
          <h1>TECH/CREATIVE<br><span class="brandSubline">SCHEDULE</span></h1>
    ...
    <div class="card authCard">
      ...
      <section id="loginPanel">
        ...
        <input id="loginId" type="text" autocomplete="username" placeholder="Enter your email or username">
        ...
        <button class="authPrimary" onclick="login()">Log In</button>
...
function renderApp(){const tabs=[['home','Home'],['calendar','Calendar'],['profile','Profile']];if(isCoordinator())tabs.push(['schedule','Schedule']);if(isAdmin())tabs.push(['admin','Admin']);appEl.innerHTML=`<div class="wrap"><div class="hero heroWithBell brandHero"><div class="brandLeft"><img src="images/tech-creative-logo.png" class="powerDinkLogo" alt="Tech/Creative logo"><span class="brandDivider"></span><div class="brandTitle"><h1>TECH/CREATIVE<br><span class="brandSubline">SCHEDULE</span></h1><p>${esc(state.profile?.name||state.profile?.username||state.user.email)} • ${esc(roleLabel(state.profile?.role))}</p></div></div></div><div class="tabs">${tabs.map(([v,l])=>`<button class="tab ${state.view===v?'active':''}" onclick="nav('${v}')">${l}</button>`).join('')}<button class="tab" onclick="logout()">Logout</button></div><main id="main"></main><div class="footer">Securely connected • Tech/Creative Schedule v1.0.0-alpha6.2.3</div></div>`;if(state.view==='calendar')renderCalendar();else if(state.view==='profile')renderProfile();else if(state.view==='schedule'&&isCoordinator())renderSchedule();else if(state.view==='admin'&&isAdmin())renderAdmin();else renderHome()}
...
function renderGridScheduler(){
  ...
  $('#scheduleManagerContent').innerHTML=`<div class="gridScheduler">
    ...
    <div class="gridPrintHeader">
      <div class="gridPrintChurch">Tech/Creative Schedule</div>
...
function gridMobileVolunteerField(service,role){
  ...
  return `<label class="mobileRoleField">
    ...
    <select aria-label="${esc(role.name)} volunteer for ${esc(service.title||'service')}" onchange="setGridAssignment('${service.id}','${role.id}',this.value)">
      ...
      ${volunteers.map(u=>`<option value="${u.id}" ${u.id===currentId?'selected':''}>${esc(u.name||u.email||'Volunteer')}</option>`).join('')}
    </select>
  </label>`;
}
